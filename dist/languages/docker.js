...
class DockerLanguage {
    ...
    async update(strategy) {
        try {
            ...
            if (!this.shell.commandExists("docker")) {
                this.logger.log("ℹ️  Docker not installed - checking images via API only");
            }
            const files = this.shell.findFiles("Dockerfile*", ".");
            if (files.length === 0) {
    ...
    updateDockerfile(dockerfile) {
        try {
            ...
            this.logger.warning(`⚠️  Failed to process ${dockerfile}: ${error.message}`);
            return false;
        }
    }
    async test() {
    ...
    async audit() {
        try {
            if (!this.shell.commandExists("trivy")) {
                ...
                this.logger.info("   Install with: brew install aquasecurity/trivy/trivy");
                return true;
            }
...
exports.DockerLanguage = DockerLanguage;
