...
import { GitManager, GitConfig } from "./GitManager";
...
export class DependencyManager {
  ...
  async executeFullUpdate(
    ...
    try {
      ...
      const strategy =
        options?.strategy ||
        (this.config.get("updateStrategy") as UpdateStrategy);
      ...
      if (
        options?.auditAfterUpdate !== false &&
  ...
  async detectOnly(): Promise<Map<string, boolean>> {
    this.logger.section("🔍 Detecting project languages");
    return this.languages.detectAll();
  }

  async updateOnly(strategy: UpdateStrategy = "minor"): Promise<UpdateResult> {
  ...
  async testOnly(): Promise<UpdateResult> {
    this.logger.section("🧪 Running tests");
    const detections = await this.detectOnly();
    return this.languages.testAll(detections);
  }

  async auditOnly(): Promise<UpdateResult> {
  ...
  private createPullRequest(): boolean {
  ...
  private printSummary(result: UpdateResult, startTime: number): void {
  ...
  private createFailureResult(): UpdateResult {
    return {
      success: false,
      updated: [],
      failed: [],
